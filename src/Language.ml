  ...
  struct
    ...
    *)
    ostap (                                      
      ...
      primary: parse_var | parse_const | parse_call | parse_parens;
...
module Stmt =
  struct
    ...
    *)
    ...
    let rec eval env conf k stmt : Expr.config = 
            let (<|>) s1 s2 = 
                    match s2 with
                    | Skip -> s1
                    | _ -> Seq (s1, s2)
    ...
    ostap (
      expr: !(Expr.parse);
      simple_stmt: read | write | assign | skip | if_stmt | while_stmt | repeat_stmt | for_stmt | call | return;
